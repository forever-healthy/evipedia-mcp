...
3. **Search** — call `search_reviews` with `query: "rapamycin"`. Show the matching review(s) and their permalinks. Then run one more search of the user's choice if they named a topic in the `/demo` args (e.g. `/demo creatine`); otherwise skip the second search.
...
5. **Conclusion only** — take the top permalink from step 3 and call `get_conclusion` on it. Show the plain-text conclusion. Point out this is the quick-answer path versus the full review in step 6.

6. **Full review** — call `get_review` on the same permalink. Don't dump the whole Markdown; show the first ~15 lines (frontmatter + opening) and note the total length, so the viewer sees it's the complete raw review.
...
**Arguments:** an optional topic (e.g. `/demo creatine`) used as the extra search in step 3 and, if it resolves to a review, as the permalink for steps 5–6 instead of rapamycin.
